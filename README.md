...
failure isolation, since upstreams fail independently. **[`docs/REFLECTOR-API.md`](docs/REFLECTOR-API.md)
...
```bash
uv run hamcall-db-reflectors --out site/api/v1 --dist dist/
uv run hamcall-db-reflectors --out site/api/v1 --skip-dvref   # no token needed
```
